...
class raw_env(SimpleEnv, EzPickle):
    ...
    def get_pos(self):
        return [agent.state.p_pos for agent in self.world.agents if not agent.adversary]
...
class Scenario(BaseScenario):
    ...
    def observation(self, agent, world):
        ...
        if not agent.adversary:
            # Good agent: [goal_rel_pos(2), landmark_rel_pos(N*2)]
            # Other agent positions EXCLUDED — GNN is the sole comm channel
            return np.concatenate(
                [agent.goal_a.state.p_pos - agent.state.p_pos]
                + entity_pos
            )
        else:
            # Adversary: full global observations — it's a pretrained/expert policy
            # Sees all landmarks + all good agent positions
